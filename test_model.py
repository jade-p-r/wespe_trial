...
print(IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_SIZE)
...
with tf.Session(config=config) as sess:
    ...
    test_dir = dped_dir + "/test_data/full_size_test_images/"
    print(test_dir)
    print(dped_dir)
    ...
    if phone.endswith("_orig"):
        ...
        for photo in test_photos:
            ...
            image = np.float16(Image.open(test_dir + photo).resize((2048, 1536))) / 255

            photo_name = photo.rsplit(".", 1)[0]

            imageio.imwrite("visual_results/" + phone + "_" + photo_name + "_original.png", image)
            print(resolution, res_sizes, res_sizes[phone])
        ...
        for i in tqdm(iteration):
            ...
            for photo in test_photos:
                ...
                enhanced_2d = sess.run(enhanced, feed_dict={x_: image_crop_2d})
                enhanced_image = np.reshape(enhanced_2d, [IMAGE_HEIGHT, IMAGE_WIDTH, 3])
                print(np.shape(enhanced_image), np.shape(image_crop))
                ...
                imageio.imsave("visual_results/" + phone + "_" + photo_name + "_iteration_" + str(i) + "_enhanced.png", enhanced_image)
                imageio.imsave("visual_results/" + phone + "_" + photo_name + "_iteration_" + str(i) + "_before_after.png", before_after)
